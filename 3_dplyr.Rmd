...
This is done using the "%>%" operator (the 'then' operator) which takes an object on the left and puts it into the function on the right. So "x %>% f(y)" is equivalent to "f(x, y)". The Rstudio hotkey for this operator is "shift+alt+.".
...
dplyr also has efficient join operations. It has support for left, semi, and anti joins. 
